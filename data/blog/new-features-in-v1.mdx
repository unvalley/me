...
Configure `siteMetadata.ts` with the settings that correspond with the desired analytics provider.
...
To enable, simply configure `siteMetadata.ts` comments property with the desired provider and settings as specified in the config file.
